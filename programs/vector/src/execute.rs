...
pub struct Execute<'info> {
    ...
    /// Optional second signer permitted inside sub-instructions.
    ///
    /// Some programs require the transaction fee payer to sign the instruction
    /// itself because it funds an account created as part of that instruction —
    /// SPL Governance `CastVote` is the motivating case, since the VoteRecord
    /// *is* the vote and cannot be pre-created. The Vault PDA cannot pay rent,
    /// so without this the instruction is impossible to express.
    ///
    /// This does NOT weaken the hot-wallet threat model: which pubkey may
    /// co-sign is part of `sub_ix_data`, which is bound into the digest the
    /// cold wallet signs, so a compromised broadcaster cannot introduce or
    /// substitute one. Anchor's `Signer` guarantees the key really signed.
    ///
    /// Omit it (pass the program ID) for flows that don't need it; those keep
    /// the strict vault-only rule.
...
pub fn handler(
    ...
    let co_signer = ctx.accounts.co_signer.as_ref().map(|s| s.key());
    let sub_ixs = decode_sub_instructions(&sub_ix_data)?;
    execute_sub_instructions(
        &sub_ixs,
